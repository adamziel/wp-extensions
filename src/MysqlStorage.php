...
PRIMARY KEY  (lang,k)
    ...
    {
        if (strlen($term) > WP_FTS_Language::MAX_TERM_KEY_BYTES) {
...
WHERE is_deleted = 0 AND doc_id IN ({$placeholders})",
            ...
            $args = array_merge([WP_FTS_Language::canonicalize($lang)], $doc_ids);
            $sql = $this->wpdb->prepare(
...
WHERE d.is_deleted = 0 AND dl.lang = %s AND dl.doc_id IN ({$placeholders})",
    ...
    public function get_doc(int $doc_id): ?array
    {
        ...
        $primaryLang = WP_FTS_Language::canonicalize((string) ($row->lang ?? WP_FTS_Language::DEFAULT_LANG));
        $langLengths = [];
        ...
        foreach ($lengthRows ?: [] as $lengthRow) {
            ...
            if ($length > 0) {
                $langLengths[WP_FTS_Language::canonicalize((string) $lengthRow->lang)] = $length;
            }
        }
    ...
    public function get_meta(?string $lang = null): array
    {
        if ($lang === null) {
            ...
            $rows = $this->wpdb->get_results($this->wpdb->prepare(
                ...
                WP_FTS_Language::canonicalize($lang)
            ));
        }
