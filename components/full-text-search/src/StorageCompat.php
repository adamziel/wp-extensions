...
/** Shared normalization and optional-capability helpers for posting-list storage. */
final class WP_FTS_StorageCompat
{
     ...
     *
     * Results are normalized to positive integers sorted by document id.
     *
     * @param int[] $docIds
    ...
    {
        $lengths = $storage->get_doc_lengths(
            $docIds,
    ...
    /**
     * Store normalized language-aware document metadata.
     *
     ...
     *
     * Missing or negative values are normalized to zero.
     *
     * @return array{doc_count:int,len_sum:int}
     */
    public static function get_meta(WP_FTS_Storage $storage, string $lang): array
    {
        $meta = $storage->get_meta(WP_FTS_TermNamespace::canonicalize_lang($lang));
    ...
    /**
     * Add signed metadata deltas for one language partition.
     *
     ...
     */
    public static function doc_lang_lengths(?array $doc): array
    {
        if ($doc === null || (bool) ($doc['deleted'] ?? false)) {
            return [];
        }

        return isset($doc['lang_lengths']) && is_array($doc['lang_lengths'])
            ? self::normalize_lang_lengths($doc['lang_lengths'])
            : [];
    }
    ...
    {
        if ($doc !== null && isset($doc['primary_lang']) && trim((string) $doc['primary_lang']) !== '') {
            return WP_FTS_TermNamespace::canonicalize_lang((string) $doc['primary_lang'], $fallbackLang);
        }
