...
test_case('public docs distinguish tokenizer readiness from real non-space language support', function (): void {
    $docs = (string) file_get_contents(__DIR__ . '/../README.md')
        . "\n"
        . (string) file_get_contents(__DIR__ . '/../docs/lexical-resources.md');

    assert_contains_text('generic synthetic readiness fixture', $docs, 'Public docs describe the synthetic readiness scope.');
    assert_contains_text('Real non-space language support still requires reviewed', $docs, 'Public docs keep the real non-space tokenizer boundary.');
    assert_contains_text('tokenizer resources and a production registry adapter', $docs, 'Public docs name the missing production resources and adapter.');
...
test_case('OEWN exact source candidate lock verifies while blocking import', function (): void {
    ...
    $cli = run_language_fts_source_candidate_lock_verifier($path, ['json' => true]);
    ...
    $no_ini_cli = run_language_fts_source_candidate_lock_verifier($path, [], true);
