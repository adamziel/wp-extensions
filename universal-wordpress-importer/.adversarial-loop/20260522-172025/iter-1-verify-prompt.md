...
You are NOT here to confirm the implementer's work. You are here
to find what is wrong, missing, faked, or hand-waved. Default to
FAIL until every doubt is reproducibly cleared.

## What to actually do (not just claim to do)

1. Re-derive the acceptance criteria from the task above. Do not anchor on the implementer's summary.
2. Read the screenshots under .tmp/v6-shots/loop-N/ with the Read tool — they render visually. If the open-popover state is missing, add a state to tools/screenshot-admin-flow.js and re-run.
3. Run composer test yourself and confirm the assertion count.
4. Run node tools/verify-option-30-flow.js /run/current-system/sw/bin/chromium yourself.
5. Grep for forbidden tokens ("Server path", wp-admin blue, external resources).
6. Read the actual changed regions of src/Admin/ImportAdminPage.php to verify each acceptance bullet.

## Forbidden verifier patterns (auto-FAIL)
- "Tests pass" without re-running.
- "Looks correct" without screenshot evidence or file:line citation.
- Treating the implementer's claims as fact.

## Output format

### Per-acceptance-criterion checklist
One line per criterion: `[✓/✗] <criterion>: <evidence>`

### Findings
Each numbered: Severity (CRITICAL/HIGH/MEDIUM/LOW), Title, Evidence (file:line or output), Reproduction (commands), Suggested action.
...
PASS only if every acceptance bullet is [✓] with concrete evidence, zero CRITICAL or HIGH findings, and the design genuinely reads as a polished GitHub-style picker + a discoverable directory selector. Default to FAIL.
