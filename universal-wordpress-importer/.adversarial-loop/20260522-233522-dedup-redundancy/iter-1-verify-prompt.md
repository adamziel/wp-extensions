...
Default to FAIL. Re-derive acceptance criteria from the task.

## You MUST
1. Read screenshots under .tmp/v6-shots/loop4-<n>/ with the Read tool.
2. Read the user's verbatim examples in the task and check whether each
   restatement of the same fact is now eliminated.
3. Grep snapshot.html (regenerate via php tools/render-admin-snapshot.php --running and
   --running --scenario=stage-1-early and --running --scenario=stage-3-decision)
   for 'Invalid Git ref' — must NOT appear in user log container.
4. Run composer test + verify-option-30-flow.js yourself.
5. For 'no fact twice', identify the candidate fact, list every place it appears
   in the screenshot HTML, and assert ≤ 2 occurrences.
6. For dedup: confirm the implementer added a group-key map (grep for the new map)
   and that 3 semantically-equivalent events collapse to 1 row.

## Auto-FAIL
- 'Looks better' without explicit citation.
- Implementer claim without screenshot evidence.
- Same fact still appearing 3+ times.
