...
final class ImportCommandTest extends TestCase {
	...
	/**
	 * Re-running the CLI tick recovers after PHP exits following a durable GitHub tree cursor write.
	 *
	 * @return void
	 */
	public function test_tick_rerun_recovers_after_real_fatal_exit_after_github_tree_cursor() {
		$source_url    = 'https://github.com/example/repository/tree/main/docs';
		$snapshot_path = $this->temporary_file( 'fatal-github-tree-cursor-wpdb.snapshot', '' );
		$cache_root    = $this->temporary_directory() . '/github-cache';
		$session       = ImportSession::start_for_source( $source_url, true );

		$this->wpdb->persist_to_file( $snapshot_path );
		$this->store->save( $session );

		$result = $this->run_fatal_github_tree_cursor_child_process( $snapshot_path, $session->get_id()->to_string(), $cache_root, 1700000000 );

		$this->assertSame( 126, $result['exit_code'], $result['stdout'] . $result['stderr'] );
		...
		$processing      = $locked_store->list_source_items_by_statuses( $session->get_id(), array( ImportSourceItem::STATUS_PROCESSING ), 10 );
		$events          = array_map(
			function ( ImportProgressEvent $event ) {
				return $event->get_type();
			},
			$locked_store->list_events( $session->get_id(), 500 )
		);

		$this->assertSame( ImportSession::STATUS_RUNNING, $locked_store->find( $session->get_id() )->get_status() );
		$this->assertNull( $locked_store->acquire_lock( $session->get_id(), 'before-github-ttl', 60 ) );
		...
		$fetcher         = $this->github_tree_fetcher();
		$archive_fetcher = new FakeRemoteArchiveFetcher( null, 'Archive fetcher should not be called.' );
		$cache           = new ImportCacheDirectory( $cache_root, 'unit-test' );
		$command         = new ImportCommand(
			$recovery_store,
			null,
			$this->cli,
			function ( WordPressImportSessionStore $store, ImportRunnerControls $controls ) use ( $archive_fetcher, $fetcher, $cache ) {
				return new ImportRunner( $store, 'wp-cli', 60, $controls, new FakePostGateway(), null, null, $archive_fetcher, $fetcher, null, $cache );
			}
		);
	...
	/**
	 * Builds a fake GitHub tree/blob fixture with one page of pending cursor work.
	 *
	 * @return FakeRemoteContentFetcher
	 */
	private function github_tree_fetcher() {
		$fetcher      = new FakeRemoteContentFetcher();
		$tree_entries = array();
	...
	/**
	 * Counts source items queued from the GitHub tree/blob API.
	 *
	 * @param WordPressImportSessionStore $store   Store to inspect.
	 * @param ImportSession               $session Session.
	 * @return int
	 */
	private function count_github_tree_children( WordPressImportSessionStore $store, ImportSession $session ) {
		$count = 0;
		$items = $store->list_source_items_by_statuses(
			$session->get_id(),
			array(
...
'
	...
	/**
	 * Runs one CLI tick in a child PHP process that exits after a GitHub tree cursor write.
	 *
	 * @param string $snapshot_path Persisted fake database snapshot path.
	 * @param string $session_id    Import session id.
	 * @param string $cache_root    Shared cache root for GitHub blob files.
	 * @param int    $now           Child process timestamp.
	 * @return array{exit_code:int,stdout:string,stderr:string}
	 */
	private function run_fatal_github_tree_cursor_child_process( $snapshot_path, $session_id, $cache_root, $now ) {
		$repository_root = dirname( __DIR__, 3 );
		$root_literal    = "'" . addcslashes( $repository_root, "\\'" ) . "'";
		$script          = $this->temporary_file(
			'fatal-github-tree-cursor-child.php',
			'<?php
$root = ' . $root_literal . ';
require_once $root . "/vendor/autoload.php";
require_once $root . "/tests/bootstrap.php";
...
$archive_fetcher = new FakeRemoteArchiveFetcher( null, "Archive fetcher should not be called." );
$cache = new ImportCacheDirectory( $cache_root, "unit-test" );
$factory = function ( WordPressImportSessionStore $store, ImportRunnerControls $controls ) use ( $archive_fetcher, $fetcher, $cache ) {
	return new ImportRunner( $store, "wp-cli", 60, $controls, new FakePostGateway(), null, null, $archive_fetcher, $fetcher, null, $cache );
};
$command = new ImportCommand( $store, null, new FakeCli(), $factory );
$command->tick( array( $session_id ), array( "simulate-fatal-after-github-tree-cursor" => true ) );
exit( 99 );
'
		);
		$command         = implode(
