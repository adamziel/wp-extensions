...
configuration. WordPress runtime indexing, REST/admin search, and WP-CLI use
`WP_FTS_Plugin::runtime_analyzer()`. Per-language lemma packs can be supplied
through the `wp_fts_analyzer_options` option or filter. Operational options
such as schema version and pending queue state are managed internally, and
...
The WP-CLI commands use the same plugin runtime analyzer as post-save indexing,
REST search, and the admin sandbox. Reindex after changing analyzer options so
stored terms and document signatures are rebuilt with the new behavior.
