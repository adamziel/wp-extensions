...
$documents = [];
$identity = 0;
for ($document = 1; $document <= WP_FTS_Relational_Storage::MAX_BATCH_DOCUMENTS; $document++) {
    $frequencies = [];
    $remainingDocuments = WP_FTS_Relational_Storage::MAX_BATCH_DOCUMENTS - $document + 1;
    $remainingIdentities = WP_FTS_Relational_Storage::MAX_BATCH_TERMS - $identity;
...
$storage = new WP_FTS_Relational_Storage(new WP_FTS_SQLite_Transport_Fixture_WPDB());
...
if (
    ...
    || ($result['identity_visits'] ?? PHP_INT_MAX) > WP_FTS_Relational_Storage::MAX_BATCH_TERMS
