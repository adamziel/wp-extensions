...
test_case_with_pdo_sqlite_fixture('relational query alternatives share their position even when analyzer output is reordered', function (): void {
    ...
    wp_fts_relational_regression_add_post($wpdb, 1, '2026-01-01 00:00:00');
    wp_fts_relational_regression_add_term($wpdb, 'surface', [1 => 100.0]);
    wp_fts_relational_regression_add_term($wpdb, 'other', [1 => 100.0]);
    wp_fts_relational_regression_add_term($wpdb, 'lemma', [1 => 100.0]);
    ...
    $payload = (new WP_FTS_Searcher($storage, $analyzer))->search('ignored', [
        'query_lang' => 'en',
        'mode' => 'AND',
        'limit' => 10,
        'post_types' => ['post'],
...
test_case_with_pdo_sqlite_fixture('relational regression keeps a signed recency cursor on its original scoring clock', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 1; $postId <= 6; $postId++) {
        wp_fts_relational_regression_add_post(
            $wpdb,
            $postId,
            gmdate('Y-m-d H:i:s', strtotime('2026-01-01 00:00:00 UTC') + ($postId * 3600))
        );
    }
    wp_fts_relational_regression_add_term($wpdb, 'clockprobe', array_fill_keys(range(1, 6), 101.0));

    $groups = wp_fts_relational_regression_groups('clockprobe');
    $options = wp_fts_relational_regression_search_options() + [
    ...
    $continuedRankSql = wp_fts_relational_regression_last_rank_sql($wpdb);
    ...
    wp_fts_relational_regression_assert_invalid_cursor(
...
test_case_with_pdo_sqlite_fixture('relational recency cursors retain posts with zero GMT dates', function (): void {
    ...
    foreach ($dates as $postId => $date) {
        wp_fts_relational_regression_add_post($wpdb, $postId, $date);
    }
    wp_fts_relational_regression_add_term($wpdb, 'zerodateprobe', array_fill_keys(array_keys($dates), 101.0));

    $groups = wp_fts_relational_regression_groups('zerodateprobe');
    ...
    $recencyExpression = new ReflectionMethod(WP_FTS_Relational_Storage::class, 'recency_score_expression');
...
test_case('relational cursor fingerprints bind one normalized range prefix', function (): void {
    $storage = new WP_FTS_Relational_Storage(new WP_FTS_Test_WPDB());
    $fingerprint = new ReflectionMethod(WP_FTS_Relational_Storage::class, 'search_cursor_fingerprint');
    $plan = [
        'groups' => [[[
...
test_case('relational cursor fingerprints are bound to one multisite index namespace', function (): void {
    $wpdb = new WP_FTS_Test_WPDB();
    $mainStorage = new WP_FTS_Relational_Storage($wpdb, 'wp_');
    $siteStorage = new WP_FTS_Relational_Storage($wpdb, 'wp_2_');
    $fingerprint = new ReflectionMethod(WP_FTS_Relational_Storage::class, 'search_cursor_fingerprint');
    $plan = [
        'groups' => [[[
...
test_case_with_pdo_sqlite_fixture('relational integerizes surface-range scores before forward and reverse cursors', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 10; $postId <= 15; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-02-01 00:00:00');
    }
    ...
    wp_fts_relational_regression_add_term($wpdb, 'prealpha', array_fill_keys(range(10, 15), 101.0));
    wp_fts_relational_regression_add_surface($wpdb, 'prealpha', array_fill_keys(range(10, 15), 101.0));

    $groups = wp_fts_relational_regression_groups('pre');
    $options = array_replace(wp_fts_relational_regression_search_options(), [
...
test_case_with_pdo_sqlite_fixture('relational regression rejects cross-query and cross-filter cursors', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 21; $postId <= 26; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-03-' . str_pad((string) ($postId - 20), 2, '0', STR_PAD_LEFT) . ' 00:00:00');
    }
    wp_fts_relational_regression_add_term($wpdb, 'queryone', array_fill_keys(range(21, 26), 100.0));
    wp_fts_relational_regression_add_term($wpdb, 'querytwo', array_fill_keys(range(21, 26), 100.0));

    $options = wp_fts_relational_regression_search_options();
    $first = $storage->search_page(wp_fts_relational_regression_groups('queryone'), $options);
    $cursor = $first['next_cursor'] ?? null;
    assert_true(is_string($cursor) && $cursor !== '', 'cross-input fixture should produce a next cursor');

    $cases = [
        'query terms' => [wp_fts_relational_regression_groups('querytwo'), $options],
        'post type' => [wp_fts_relational_regression_groups('queryone'), array_replace($options, ['post_types' => ['page']])],
        'post status' => [wp_fts_relational_regression_groups('queryone'), array_replace($options, ['post_statuses' => ['draft']])],
        'lower date boundary' => [wp_fts_relational_regression_groups('queryone'), array_replace($options, ['date_after' => '2026-03-03 00:00:00'])],
        'upper date boundary' => [wp_fts_relational_regression_groups('queryone'), array_replace($options, ['date_before' => '2026-03-05 00:00:00'])],
    ];
...
test_case_with_pdo_sqlite_fixture('relational regression rejects cursors after the durable search epoch advances', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 31; $postId <= 34; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-03-10 00:00:00');
    }
    wp_fts_relational_regression_add_term($wpdb, 'epochprobe', array_fill_keys(range(31, 34), 100.0));

    $groups = wp_fts_relational_regression_groups('epochprobe');
    $options = wp_fts_relational_regression_search_options(1);
    $first = $storage->search_page($groups, $options);
    $cursor = $first['next_cursor'] ?? null;
...
test_case_with_pdo_sqlite_fixture('relational search statements fail closed when publication changes between bounded reads', function (): void {
    $search = static function (string $mutateBefore): array {
        [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
        wp_fts_relational_regression_add_post($wpdb, 35, '2026-03-10 00:00:00');
        wp_fts_relational_regression_add_term($wpdb, 'snapshotrace', [35 => 100.0]);
        $options = array_replace(wp_fts_relational_regression_search_options(1), [
        ...
        $error = null;
        try {
            $storage->search_page(wp_fts_relational_regression_groups('snapshotrace'), $options);
...
test_case_with_pdo_sqlite_fixture('relational search uses one authoritative sentinel per statement including valid zero hits', function (): void {
    ...
    wp_fts_relational_regression_add_term($wpdb, 'snapshotzerohit', [36 => 100.0]);

    $wpdb->queries = [];
    $page = $storage->search_page(
        wp_fts_relational_regression_groups('snapshotzerohit'),
    ...
    wp_fts_relational_regression_add_post($hydrationWpdb, 37, '2026-03-10 00:00:00');
    wp_fts_relational_regression_add_term($hydrationWpdb, 'snapshothydrate', [37 => 100.0]);
    $hydrationWpdb->queries = [];
    $hydrationStorage->search_page(
        wp_fts_relational_regression_groups('snapshothydrate'),
...
test_case_with_pdo_sqlite_fixture('relational planning rejects malformed database transport', function (): void {
    $mutations = [
        'missing alias' => static function (array $rows): array {
            unset($rows[0]->doc_freq);
            return $rows;
        },
        'noncanonical epoch' => static function (array $rows): array {
            $rows[0]->search_epoch = '01';
            return $rows;
        },
        'unknown row kind' => static function (array $rows): array {
            $rows[0]->row_kind = 'ignored';
            return $rows;
        },
        'duplicate requested identity' => static function (array $rows): array {
            $rows[1]->lang = $rows[0]->lang;
            $rows[1]->term = $rows[0]->term;
            return $rows;
        },
        'inconsistent publication epoch' => static function (array $rows): array {
            $rows[1]->search_epoch = (int) $rows[0]->search_epoch + 1;
            return $rows;
        },
    ];
    ...
    foreach ($mutations as $label => $mutate) {
        [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
        wp_fts_relational_regression_add_post($wpdb, 701, '2026-03-10 00:00:00');
        wp_fts_relational_regression_add_term($wpdb, 'transportalpha', [701 => 100.0]);
        wp_fts_relational_regression_add_term($wpdb, 'transportbeta', [701 => 100.0]);
        $wpdb->queries = [];
        $wpdb->readResultMutator = static function (string $sql, array $rows) use ($mutate): array {
            return str_contains($sql, '/* wp_fts:plan */') ? $mutate($rows) : $rows;
        };
        $error = null;
        try {
            $storage->search_page([[
                ['key' => WP_FTS_TermNamespace::namespace_term('en', 'transportalpha'), 'rank' => 0],
                ['key' => WP_FTS_TermNamespace::namespace_term('en', 'transportbeta'), 'rank' => 1],
            ]], wp_fts_relational_regression_search_options(1));
        } catch (WP_FTS_Search_Unavailable $caught) {
            $error = $caught;
        }

        assert_true($error instanceof WP_FTS_Search_Unavailable, "{$label} planning transport must fail closed");
        assert_contains('malformed', strtolower($error?->getMessage() ?? ''), "{$label} should be reported as malformed planning transport");
        assert_same(1, count($wpdb->queries), "{$label} must stop before ranking");
    }
});

test_case_with_pdo_sqlite_fixture('relational ranking rejects malformed database transport', function (): void {
    $mutations = [
        'missing score' => static function (array $rows): array {
            unset($rows[0]->score);
            return $rows;
        },
        'noncanonical document id' => static function (array $rows): array {
            $rows[0]->doc_id = '0702';
            return $rows;
        },
        'fractional score' => static function (array $rows): array {
            $rows[0]->score = 1.5;
            return $rows;
        },
        'invalid date' => static function (array $rows): array {
            $rows[0]->post_date_gmt = '2026-02-30 00:00:00';
            return $rows;
        },
        'missing canonical byte count' => static function (array $rows): array {
            unset($rows[0]->canonical_post_bytes);
            return $rows;
        },
        'duplicate document id' => static function (array $rows): array {
            $rows[] = clone $rows[0];
            return $rows;
        },
        'result mixed with control row' => static function (array $rows): array {
            $control = clone $rows[0];
            $control->doc_id = null;
            $control->score = null;
            $control->post_date_gmt = null;
            $rows[] = $control;
            return $rows;
        },
    ];
    ...
    foreach ($mutations as $label => $mutate) {
        [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
        wp_fts_relational_regression_add_post($wpdb, 702, '2026-03-10 00:00:00');
        wp_fts_relational_regression_add_term($wpdb, 'ranktransport', [702 => 100.0]);
        $wpdb->queries = [];
        $wpdb->readResultMutator = static function (string $sql, array $rows) use ($mutate): array {
            return str_contains($sql, '/* wp_fts:rank */') ? $mutate($rows) : $rows;
        };
        $error = null;
        try {
            $storage->search_page(
                wp_fts_relational_regression_groups('ranktransport'),
                array_replace(wp_fts_relational_regression_search_options(1), ['include_metadata' => true])
            );
        } catch (WP_FTS_Search_Unavailable $caught) {
            $error = $caught;
        ...
        assert_true($error instanceof WP_FTS_Search_Unavailable, "{$label} ranking transport must fail closed");
        assert_contains('malformed', strtolower($error?->getMessage() ?? ''), "{$label} should be reported as malformed ranking transport");
        assert_same(2, count($wpdb->queries), "{$label} must stop before hydration");
    }
});

test_case_with_pdo_sqlite_fixture('relational ranking accepts exact integral float transport', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    wp_fts_relational_regression_add_post($wpdb, 705, '2026-03-10 00:00:00');
    wp_fts_relational_regression_add_term($wpdb, 'floattransport', [705 => 100.0]);
    $wpdb->readResultMutator = static function (string $sql, array $rows): array {
        if (str_contains($sql, '/* wp_fts:rank */')) {
            $rows[0]->score = (float) $rows[0]->score;
        }
        return $rows;
    };

    $page = $storage->search_page(
        wp_fts_relational_regression_groups('floattransport'),
        wp_fts_relational_regression_search_options(1)
    );
    assert_same([705], array_column($page['results'], 'doc_id'), 'an exact integral driver float should preserve its ranked document');
    assert_true(is_float($page['results'][0]['score'] ?? null), 'the public score should retain its numeric result contract');
});

test_case_with_pdo_sqlite_fixture('relational hydration rejects malformed database transport', function (): void {
    $mutations = [
        'missing requested alias' => static function (array $rows): array {
            unset($rows[0]->post_title);
            return $rows;
        },
        'unknown document id' => static function (array $rows): array {
            $rows[0]->doc_id = 999999;
            return $rows;
        },
        'duplicate document id' => static function (array $rows): array {
            $rows[] = clone $rows[0];
        ...
        'missing requested document' => static function (array $rows): array {
            array_pop($rows);
            return $rows;
        },
        'invalid text type' => static function (array $rows): array {
            $rows[0]->post_title = ['not', 'text'];
            return $rows;
        },
    ];
    ...
    foreach ($mutations as $label => $mutate) {
        [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
        wp_fts_relational_regression_add_post($wpdb, 703, '2026-03-10 00:00:00');
        wp_fts_relational_regression_add_post($wpdb, 704, '2026-03-09 00:00:00');
        wp_fts_relational_regression_add_term($wpdb, 'hydratetransport', [703 => 100.0, 704 => 100.0]);
        $wpdb->queries = [];
        $wpdb->readResultMutator = static function (string $sql, array $rows) use ($mutate): array {
            return str_contains($sql, '/* wp_fts:hydrate */') ? $mutate($rows) : $rows;
        };
        $error = null;
        try {
            $storage->search_page(
                wp_fts_relational_regression_groups('hydratetransport'),
                array_replace(wp_fts_relational_regression_search_options(2), ['include_metadata' => true])
            );
        } catch (WP_FTS_Search_Unavailable $caught) {
            $error = $caught;
        }

        assert_true($error instanceof WP_FTS_Search_Unavailable, "{$label} hydration transport must fail closed");
        assert_contains('malformed', strtolower($error?->getMessage() ?? ''), "{$label} should be reported as malformed hydration transport");
        assert_same(3, count($wpdb->queries), "{$label} must stop at hydration");
    }
});

test_case_with_pdo_sqlite_fixture('relational cursors cannot cross a capability publication with an unchanged epoch', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 38; $postId <= 41; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-03-10 00:00:00');
    }
    wp_fts_relational_regression_add_term($wpdb, 'capabilitycursor', array_fill_keys(range(38, 41), 100.0));
    $oldOptions = wp_fts_relational_regression_search_options(1);
    $first = $storage->search_page(wp_fts_relational_regression_groups('capabilitycursor'), $oldOptions);
    $cursor = $first['next_cursor'] ?? null;
    ...
    wp_fts_relational_regression_assert_invalid_cursor(
        static fn() => $storage->search_page(
            wp_fts_relational_regression_groups('capabilitycursor'),
...
test_case_with_pdo_sqlite_fixture('relational validates cursors before every empty or impossible search return', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 41; $postId <= 44; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-03-11 00:00:00');
    }
    wp_fts_relational_regression_add_term($wpdb, 'cursorpresent', array_fill_keys(range(41, 44), 100.0));

    $options = wp_fts_relational_regression_search_options(1);
    $presentGroups = wp_fts_relational_regression_groups('cursorpresent');
    ...
    wp_fts_relational_regression_assert_invalid_cursor(
        static fn() => $storage->search_page(
            wp_fts_relational_regression_groups('cursorabsent'),
    ...
    wp_fts_relational_regression_assert_invalid_cursor(
        static fn() => (new WP_FTS_Searcher($storage, $emptyAnalyzer))->search(
            'ignored',
...
test_case_with_pdo_sqlite_fixture('relational planning cardinality is independent of adversarial lexical neighbors', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    wp_fts_relational_regression_add_post($wpdb, 51, '2026-03-12 00:00:00');
    wp_fts_relational_regression_add_term($wpdb, 'collisiontarget', [51 => 100.0]);
    ...
    $payload = $storage->search_page(
        wp_fts_relational_regression_groups('collisiontarget'),
        wp_fts_relational_regression_search_options(1)
    );
...
test_case_with_pdo_sqlite_fixture('relational pagination advances across a full K+1 window of oversized canonical rows', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 61; $postId <= 63; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-03-13 00:00:00');
    }
    wp_fts_relational_regression_add_term($wpdb, 'oversizedcanonical', array_fill_keys(range(61, 63), 100.0));
    $wpdb->rankCanonicalByteOverrides = [
        63 => WP_FTS_Relational_Storage::MAX_CANONICAL_POST_BYTES + 1,
        62 => WP_FTS_Relational_Storage::MAX_CANONICAL_POST_BYTES + 1,
    ];
    $options = array_replace(wp_fts_relational_regression_search_options(1), ['include_metadata' => true]);

    $wpdb->queries = [];
    $first = $storage->search_page(wp_fts_relational_regression_groups('oversizedcanonical'), $options);
    assert_same([], $first['results'], 'oversized canonical rows should not be partially hydrated or returned');
    ...
    $second = $storage->search_page(
        wp_fts_relational_regression_groups('oversizedcanonical'),
    ...
    [$reverseWpdb, $reverseStorage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 71; $postId <= 75; $postId++) {
        wp_fts_relational_regression_add_post($reverseWpdb, $postId, '2026-03-14 00:00:00');
    }
    wp_fts_relational_regression_add_term($reverseWpdb, 'oversizedreverse', array_fill_keys(range(71, 75), 100.0));
    $reverseWpdb->rankCanonicalByteOverrides = [
        74 => WP_FTS_Relational_Storage::MAX_CANONICAL_POST_BYTES + 1,
        73 => WP_FTS_Relational_Storage::MAX_CANONICAL_POST_BYTES + 1,
    ];
    $reverseGroups = wp_fts_relational_regression_groups('oversizedreverse');
    $reverseOptions = array_replace(wp_fts_relational_regression_search_options(1), ['include_metadata' => true]);
...
test_case_with_pdo_sqlite_fixture('relational ranges over one normalized surface per indexed token', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    wp_fts_relational_regression_add_post($wpdb, 900, '2026-04-01 00:00:00');
    wp_fts_relational_regression_add_term($wpdb, 'construction', [900 => 0.001]);
    wp_fts_relational_regression_add_surface($wpdb, 'construction', [900 => 0.001]);
    for ($ordinal = 0; $ordinal <= 256; $ordinal++) {
        $term = 'construction' . str_pad((string) $ordinal, 3, '0', STR_PAD_LEFT);
        $postId = 1000 + $ordinal;
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-04-01 00:00:00');
        $impact = $ordinal === 256 ? 10000.0 : 100.0;
        wp_fts_relational_regression_add_term($wpdb, $term, [$postId => $impact]);
        wp_fts_relational_regression_add_surface($wpdb, $term, [$postId => $impact]);
    }

    $groups = wp_fts_relational_regression_groups('construction');
    $exactPayload = $storage->search_page($groups, wp_fts_relational_regression_search_options(10));
    ...
    $options = array_replace(wp_fts_relational_regression_search_options(10), [
        'prefix_matching' => true,
        'prefix_group_index' => 0,
...
test_case('relational visibility never walks taxonomy relationships per ranked candidate', function (): void {
    $storage = new WP_FTS_Relational_Storage(new WP_FTS_Test_WPDB());
    $visibilitySql = new ReflectionMethod(WP_FTS_Relational_Storage::class, 'visibility_sql');
    $visibilitySql->setAccessible(true);
    $visibility = $visibilitySql->invoke($storage, 'ranked.post_id', 'ranked', ['post_types' => ['post']]);
    ...
    assert_contains('wp_fts_documents d_ranked FORCE INDEX (visibility)', (string) ($visibility['joins'] ?? ''), 'ranked visibility should probe the narrow document-existence index');
    assert_contains('wp_posts wp_ranked FORCE INDEX (`wp_fts_type_status_id`)', (string) ($visibility['joins'] ?? ''), 'typed ranked visibility should stay on the covering WordPress index');
    ...
    $hydration = $visibilitySql->invoke($storage, 'rank_ids.post_id', 'hydrated', ['post_types' => ['post']], true);
    assert_true(!str_contains((string) ($hydration['joins'] ?? ''), 'd_hydrated FORCE INDEX (visibility)'), 'hydration must retain access to document language and snippet payload');
    assert_contains('wp_posts wp_hydrated FORCE INDEX (`wp_fts_type_status_id`)', (string) ($hydration['joins'] ?? ''), 'bounded hydration may retain the covering visibility probe before reading selected post fields');

    $untyped = $visibilitySql->invoke($storage, 'ranked.post_id', 'untyped', []);
    assert_true(!str_contains((string) ($untyped['joins'] ?? ''), 'wp_untyped FORCE INDEX'), 'an all-post-type search must not force an index whose leading post_type key is unconstrained');
});
...
test_case_with_pdo_sqlite_fixture('relational real Russian ambiguity ranges on the typed surface without replacing exact lemmas', function (): void {
    $manifest = dirname(__DIR__, 2) . '/resources/analyzer-packs/ru-unimorph-rus-50dcabfd0a04/manifest.json';
    $analyzer = new WP_FTS_Analyzer([
        'default_lang' => 'ru',
        'auto_detect_language' => false,
    ...
    foreach ($terms as $postId => $term) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-04-01 00:00:00');
        wp_fts_relational_regression_add_term($wpdb, $term, [$postId => 100.0], 'ru');
        wp_fts_relational_regression_add_surface($wpdb, $term, [$postId => 100.0], 'ru');
    }
...
test_case_with_pdo_sqlite_fixture('relational AND prefixes intersect one range-led scan with exact candidates', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 1; $postId <= 100; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-04-02 00:00:00');
    }
    wp_fts_relational_regression_add_term($wpdb, 'commonanchor', array_fill_keys(range(1, 100), 100.0));
    wp_fts_relational_regression_add_term($wpdb, 'rareprecompletion', [1 => 100.0, 2 => 100.0]);
    wp_fts_relational_regression_add_surface($wpdb, 'rareprecompletion', [1 => 100.0, 2 => 100.0]);
    wp_fts_relational_regression_add_term($wpdb, 'lemmaoutside', [3 => 200.0]);
    ...
    $wpdb->queries = [];
    $payload = $storage->search_page($groups, array_replace(wp_fts_relational_regression_search_options(10), [
        'mode' => 'AND',
        'prefix_matching' => true,
    ...
    wp_fts_relational_regression_add_surface($wpdb, 'selectivecompletion', [1 => 100.0, 2 => 300.0, 3 => 200.0]);
    $wpdb->queries = [];
    $selectiveGroups = [
        [['key' => WP_FTS_TermNamespace::namespace_term('en', 'commonanchor'), 'rank' => 0]],
        [['key' => WP_FTS_TermNamespace::namespace_term('en', 'selective'), 'rank' => 0]],
    ];
    $selectiveOptions = array_replace(wp_fts_relational_regression_search_options(2), [
        'mode' => 'AND',
        'prefix_matching' => true,
        'prefix_group_index' => 1,
    ...
    $selectiveRankSql = wp_fts_relational_regression_last_rank_sql($wpdb);
...
test_case_with_pdo_sqlite_fixture('relational broad non-anchor prefixes execute candidate-first with exact score', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    $wpdb->dbh->beginTransaction();
    for ($postId = 1; $postId <= 8193; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-04-02 00:00:00');
    }
    wp_fts_relational_regression_add_term($wpdb, 'candidateanchor', [1 => 100.0]);
    wp_fts_relational_regression_add_surface(
        $wpdb,
        'broadprefixcompletion',
        array_fill_keys(range(1, 8193), 200.0)
    );
    for ($term = 0; $term < 32; $term++) {
        wp_fts_relational_regression_add_term($wpdb, 'unrelated' . $term, [1 => 300.0]);
        wp_fts_relational_regression_add_surface($wpdb, 'unrelated' . $term, [1 => 300.0]);
    }
    $wpdb->dbh->commit();
...
test_case_with_pdo_sqlite_fixture('relational unavailable surface ranges use exact probes without weakening cursor identity', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    foreach ([1, 2, 3] as $postId) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-04-02 00:00:00');
    }
    wp_fts_relational_regression_add_term($wpdb, 'commonanchor', [1 => 100.0, 2 => 100.0, 3 => 100.0]);
    wp_fts_relational_regression_add_term($wpdb, 'nosurface', [1 => 100.0, 2 => 100.0]);
...
test_case('relational MySQL rank arms pin bounded drivers ahead of postings and visibility', function (): void {
    ...
    assert_contains('STRAIGHT_JOIN wp_fts_documents d_exact_anchor FORCE INDEX (visibility) ON', $exactSql, 'the rare posting candidate must drive its narrow document-existence probe');
    assert_contains('STRAIGHT_JOIN wp_posts wp_exact_anchor FORCE INDEX (`wp_fts_type_status_id`) ON', $exactSql, 'the indexed candidate must drive its covering canonical visibility probe');
...
test_case_with_pdo_sqlite_fixture('relational overlap costing uses dictionary DF only and never scans postings during planning', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    for ($postId = 1; $postId <= 60; $postId++) {
        wp_fts_relational_regression_add_post($wpdb, $postId, '2026-04-03 00:00:00');
    }
    $overlap = array_fill_keys(range(1, 40), 100.0);
    wp_fts_relational_regression_add_term($wpdb, 'overlapalpha', $overlap);
    wp_fts_relational_regression_add_term($wpdb, 'overlapbeta', $overlap);
    wp_fts_relational_regression_add_term($wpdb, 'sixtyanchor', array_fill_keys(range(1, 60), 100.0));
    ...
    $wpdb->queries = [];
    $payload = $storage->search_page($groups, array_replace(wp_fts_relational_regression_search_options(50), [
        'mode' => 'AND',
        'explain' => true,
    ]));
...
test_case_with_pdo_sqlite_fixture('relational regression fences an explicit retry that arrives during an active lease', function (): void {
    ...
    $queue->retry_many([71], 1001);
    $row = wp_fts_relational_regression_work_row($wpdb, 71);
    ...
    assert_same(0, $queue->acknowledge_many([$old])['acknowledged'], 'the stale lease must not acknowledge the retried generation');
    assert_same(0, $queue->fail_many([$old], 1002), 'the stale lease must not defer the retried generation');
    assert_same(0, $queue->release_many([$old], 1002), 'the stale lease must not release or rewrite the retried generation');
    ...
    assert_same(1, $queue->acknowledge_many([$new])['acknowledged'], 'the new generation owner should retain acknowledgement rights');
    assert_same(0, $queue->count(), 'only the new generation owner should remove the work row');
});

test_case_with_pdo_sqlite_fixture('relational SQLite mutation fences recover and supersede by generation CAS', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $queue = new WP_FTS_Index_Queue($wpdb);
    ...
    $fenced = wp_fts_relational_regression_work_row($wpdb, 72);
    ...
    assert_same(1, $queue->acknowledge_many([$recovered])['acknowledged'], 'the exact recovered SQLite generation should be acknowledgeable');
    assert_same(0, $queue->acknowledge_many([$recovered])['acknowledged'], 'one recovered generation must not acknowledge twice');
    ...
    $superseded = wp_fts_relational_regression_work_row($wpdb, 73);
    ...
    $promoted = wp_fts_relational_regression_work_row($wpdb, 73);
...
test_case_with_pdo_sqlite_fixture('relational SQLite late promotions preserve postdeadline successor intent', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $queue = new WP_FTS_Index_Queue($wpdb);

    $postToken = 'guard:' . str_repeat('c', 32);
    $postPayload = ['index_options' => ['document_lang' => 'pl']];
    ...
    $postAfterPromotion = wp_fts_relational_regression_work_row($wpdb, 74);
    ...
    assert_same(0, $queue->acknowledge_many([$recoveredPost])['acknowledged'], 'the recovered post worker must not acknowledge the post-hook successor');
    $postAfterStaleAcknowledge = wp_fts_relational_regression_work_row($wpdb, 74);
    ...
    assert_same(1, $queue->acknowledge_many([$postSuccessor])['acknowledged'], 'the post-hook successor alone should retain acknowledgement rights');

    $scopeKey = 'sqlite-late-targeted';
    $scopeToken = 'guard:' . str_repeat('d', 32);
...
test_case_with_pdo_sqlite_fixture('relational SQLite foreground handoff releases owned fences without tombstones or accidental global scopes', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $queue = new WP_FTS_Index_Queue($wpdb);
    $postToken = str_repeat('c', 32);
    ...
    $post = wp_fts_relational_regression_work_row($wpdb, 740);
    ...
    for ($offset = 0; $offset < 25; $offset++) {
        $postId = 800 + $offset;
        $postLoopToken = substr(hash('sha256', 'post-' . $offset), 0, 32);
        $scopeLoopToken = substr(hash('sha256', 'scope-' . $offset), 0, 32);
...
test_case_with_pdo_sqlite_fixture('relational SQLite rejects impossible multi-scope ownership and deletes the maximum one by primary-key CAS', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $queue = new WP_FTS_Index_Queue($wpdb);
    $oversizedTokens = [];
...
test_case_with_pdo_sqlite_fixture('relational SQLite reset rebuilds 409600 populated postings with constant metadata work', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $wpdb->dbh->beginTransaction();
    try {
    ...
    $guardCalls = 0;
    $storage = new WP_FTS_Relational_Storage($wpdb, null, static function () use (&$guardCalls): void {
        $guardCalls++;
    });
    $wpdb->queries = [];
...
test_case_with_pdo_sqlite_fixture('relational SQLite schema verification stays fixed with 2048 unrelated indexes', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    ...
    assert_same(1, count($wpdb->queries), 'detecting a duplicate engine-specific name must retain the one-statement metadata contract');
...
test_case_with_pdo_sqlite_fixture('relational real SQLite worker drains only the newest canonical generation', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    ...
    $fenced = wp_fts_relational_regression_work_row($wpdb, 741);
    ...
    $ready = wp_fts_relational_regression_work_row($wpdb, 741);
    ...
    $search = wp_fts_quality_with_wpdb($wpdb, static function () use ($wpdb): array {
        ...
        return (new WP_FTS_Searcher($storage, WP_FTS_Plugin::runtime_analyzer()))->search(
...
test_case_with_pdo_sqlite_fixture('relational SQLite schema repair is idempotent and preserves postings, work, and cursor epoch', function (): void {
    ...
    assert_same(true, $payload['before_valid'] ?? null, 'the translated SQLite schema should satisfy the current relational contract before repair');
    assert_same(true, $payload['after_valid'] ?? null, 'the translated SQLite schema should satisfy the current relational contract after repeated repair');
...
test_case('relational native schema repair drops only inspected search-generation tables', function (): void {
    $wpdb = new WP_FTS_Test_WPDB();
    $storage = new WP_FTS_Relational_Storage($wpdb);
    $drop = new ReflectionMethod(WP_FTS_Relational_Storage::class, 'drop_existing_schema_tables');
...
test_case_with_pdo_sqlite_fixture('relational scope page fan-out and cursor progress commit atomically behind the generation fence', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $queue = new WP_FTS_Index_Queue($wpdb);
...
test_case_with_pdo_sqlite_fixture('relational worker alternates direct claims and scope pages across bounded cron invocations', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $directIds = range(91, 110);
    $scopeIds = range(1, 20);
    foreach ([...$directIds, ...$scopeIds] as $postId) {
        wp_fts_relational_regression_add_source_post($wpdb, $postId, "<p>fairness source {$postId}</p>", '');
    }
...
test_case_with_pdo_sqlite_fixture('relational targeted scopes use the exact composite membership keyset', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    wp_fts_relational_regression_add_source_post($wpdb, 1, '<p>first exact target</p>', '');
    wp_fts_relational_regression_add_source_post($wpdb, 201, '<p>last exact target</p>', '');
...
test_case_with_pdo_sqlite_fixture('relational targeted scopes ignore unrelated relationship fanout', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    wp_fts_relational_regression_add_source_post($wpdb, 300, '<p>high fanout exact target</p>', '');
...
test_case_with_pdo_sqlite_fixture('relational selective keysets skip sparse gaps while corpus pages stay raw and bounded', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    ...
    wp_fts_relational_regression_add_source_post($wpdb, 202, '<p>first eligible row after sparse gap</p>', '');
...
test_case_with_pdo_sqlite_fixture('relational worker defers the 50000-posting aggregate overflow without a hot loop or lost generation', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $terms = [];
    ...
    for ($postId = 20001; $postId <= 20100; $postId++) {
        wp_fts_relational_regression_add_source_post($wpdb, $postId, $content, '');
    }
    ...
    wp_fts_quality_with_wpdb($wpdb, static function (): void {
        assert_same(100, WP_FTS_Plugin::enqueue_posts_for_reindex(range(20001, 20100), ['document_lang' => 'en']), 'fixture should durably enqueue all 100 generations');
    });
    $wpdb->queries = [];
...
test_case_with_pdo_sqlite_fixture('relational worker preserves a source-deferred suffix at the 50000-posting frontier', function (): void {
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
    $terms = [];
    ...
    for ($postId = 21001; $postId <= 21099; $postId++) {
        wp_fts_relational_regression_add_source_post($wpdb, $postId, $sharedContent, '');
    }
    ...
    WP_FTS_Plugin::reset_request_caches();
    wp_fts_quality_with_wpdb($wpdb, static function (): void {
        assert_same(100, WP_FTS_Plugin::enqueue_posts_for_reindex(range(21001, 21100), ['document_lang' => 'en']), 'fixture should enqueue every mixed-deferral generation');
    });
    $wpdb->queries = [];
...
test_case('relational writer signals the 50000-posting aggregate overflow before issuing SQL', function (): void {
    ...
    for ($postId = 1; $postId <= 100; $postId++) {
        $documents[] = [
            ...
            'snippet_text' => "100% aggregate O'Reilly post {$postId}",
    ...
    try {
        $storage->replace_prepared_documents([[
            ...
            'content_hash' => sha1('language-poison'),
            'snippet_text' => '',
            'term_frequencies' => [],
            'surface_frequencies' => [],
        ]]);
...
test_case_with_pdo_sqlite_fixture('relational search plan accepts 12x12x12 boundaries and rejects the next unit before SQL', function (): void {
    ...
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    wp_fts_relational_regression_add_post($wpdb, 1, '2026-07-18 00:00:00');
    $keys = [];
    for ($index = 0; $index < 20; $index++) {
        $term = 'planbound' . str_pad((string) $index, 2, '0', STR_PAD_LEFT);
        $keys[] = WP_FTS_TermNamespace::namespace_term('en', $term);
        wp_fts_relational_regression_add_term($wpdb, $term, [1 => 100.0]);
    }
    $options = array_replace(wp_fts_relational_regression_search_options(10), ['mode' => 'AND']);
    ...
    assert_same(
        12,
        count($wpdb->dbh->query(wp_fts_relational_regression_last_plan_sql($wpdb))->fetchAll(PDO::FETCH_OBJ)),
    ...
    wp_fts_relational_regression_add_surface($wpdb, 'planbound00', [1 => 100.0]);
    ...
    $prefixPlanSql = wp_fts_relational_regression_last_plan_sql($wpdb);
    ...
    $maximumAndPlanSql = wp_fts_relational_regression_last_plan_sql($wpdb);
...
test_case_with_pdo_sqlite_fixture('relational direct AND plans reject malformed groups without widening', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    wp_fts_relational_regression_add_post($wpdb, 1, '2026-07-18 00:00:00');
    wp_fts_relational_regression_add_term($wpdb, 'validplanarm', [1 => 100.0]);
    $validGroup = wp_fts_relational_regression_groups('validplanarm')[0];
    $options = array_replace(wp_fts_relational_regression_search_options(10), ['mode' => 'AND']);
...
test_case_with_pdo_sqlite_fixture('relational direct metadata filters reject malformed restrictions without widening', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    wp_fts_relational_regression_add_post($wpdb, 1, '2026-07-18 00:00:00');
    wp_fts_relational_regression_add_term($wpdb, 'filterfailclosed', [1 => 100.0]);
    $groups = wp_fts_relational_regression_groups('filterfailclosed');
    $options = wp_fts_relational_regression_search_options(10);
...
test_case_with_pdo_sqlite_fixture('relational direct storage options reject unknown keys and malformed values before SQL', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();
    wp_fts_relational_regression_add_post($wpdb, 1, '2026-07-18 00:00:00');
    wp_fts_relational_regression_add_term($wpdb, 'optionfailclosed', [1 => 100.0]);
    $groups = wp_fts_relational_regression_groups('optionfailclosed');
    $options = wp_fts_relational_regression_search_options(10);
    ...
    try {
        $knownOptions = [
            'mode', 'page_size', 'cursor', 'direction',
    ...
    foreach (['limit', 'highlight', 'snippet_length'] as $presentationOption) {
        $wpdb->queries = [];
        $error = null;
        try {
            $storage->search_page($groups, array_replace($options, [$presentationOption => 1]));
        } catch (InvalidArgumentException $caught) {
            $error = $caught;
        }
        assert_true($error instanceof InvalidArgumentException, "direct storage should reject presentation option {$presentationOption}");
        assert_same([], $wpdb->queries, "unsupported {$presentationOption} must reject before planning SQL");
    }
});

test_case_with_pdo_sqlite_fixture('relational backend implements set-oriented search and prepared-document writes', function (): void {
    [$wpdb, $storage] = wp_fts_relational_regression_search_fixture();

    assert_true($storage instanceof WP_FTS_Set_Oriented_Search_Storage, 'the relational backend should expose database-owned planning, ranking, and pagination');

    $termKey = WP_FTS_TermNamespace::namespace_term('en', 'writerprobe');
    $result = $storage->replace_prepared_documents([[
        'doc_id' => 991,
        'primary_lang' => 'en',
        'content_hash' => sha1('writerprobe'),
        'snippet_text' => '',
...
/** @return array{0:WP_FTS_Relational_Regression_SQLite_WPDB,1:WP_FTS_Relational_Storage} */
function wp_fts_relational_regression_search_fixture(): array
{
    $wpdb = new WP_FTS_Relational_Regression_SQLite_WPDB();
    wp_fts_relational_regression_create_schema($wpdb);
...
function wp_fts_relational_regression_create_schema(WP_FTS_Relational_Regression_SQLite_WPDB $wpdb): void
{
    $statements = [
        ...
        'CREATE INDEX wp_fts_documents_visibility ON wp_fts_documents(post_id)',
    ...
    foreach ([
        ...
        WP_FTS_Plugin::SEARCH_READY_INCARNATION_OPTION => serialize([
            ...
            'profile_hash' => wp_fts_relational_regression_ready_profile_hash(),
        ]),
    ] as $name => $value) {
    ...
    $wpdb->execute(
        ...
        [WP_FTS_Index_Queue::SEARCH_EPOCH_JOB_KEY, 'meta', 0, 1, 'meta', wp_fts_relational_regression_epoch_incarnation()]
    );
}

/** Seed one canonical and indexed row so visibility is tested through real SQL. */
function wp_fts_relational_regression_add_post(
    WP_FTS_Relational_Regression_SQLite_WPDB $wpdb,
    int $postId,
    string $date,
...
function wp_fts_relational_regression_add_source_post(
    WP_FTS_Relational_Regression_SQLite_WPDB $wpdb,
    int $postId,
    string $content,
...
function wp_fts_relational_regression_add_term(WP_FTS_Relational_Regression_SQLite_WPDB $wpdb, string $term, array $impactsByPost, string $lang = 'en'): void
{
    wp_fts_relational_regression_add_dictionary_identity($wpdb, $term, $impactsByPost, $lang, 0);
}

/** @param array<int,float> $impactsByPost */
function wp_fts_relational_regression_add_surface(WP_FTS_Relational_Regression_SQLite_WPDB $wpdb, string $term, array $impactsByPost, string $lang = 'en'): void
{
    wp_fts_relational_regression_add_dictionary_identity($wpdb, $term, $impactsByPost, $lang, 1);
}

/** @param array<int,float> $impactsByPost */
function wp_fts_relational_regression_add_dictionary_identity(
    WP_FTS_Relational_Regression_SQLite_WPDB $wpdb,
    string $term,
    array $impactsByPost,
    string $lang,
...
function wp_fts_relational_regression_groups(string $term): array
{
    return [[['key' => WP_FTS_TermNamespace::namespace_term('en', $term), 'rank' => 0]]];
}

/** @return array<string,mixed> */
function wp_fts_relational_regression_search_options(int $pageSize = 2): array
{
    return [
        'mode' => 'OR',
        'page_size' => $pageSize,
        ...
        'post_statuses' => ['publish'],
        'prefix_matching' => false,
        'search_ready_incarnation' => wp_fts_relational_regression_ready_incarnation(),
        'search_ready_profile_hash' => wp_fts_relational_regression_ready_profile_hash(),
    ];
}

/** Keep the published readiness generation distinct from the epoch payload. */
function wp_fts_relational_regression_ready_incarnation(): string
{
    return str_repeat('a', 32);
}

/** Provide a stable analyzer-profile binding for cursor and readiness checks. */
function wp_fts_relational_regression_ready_profile_hash(): string
{
    return str_repeat('b', 40);
}

/** Deliberately separate the epoch incarnation from the publication token. */
function wp_fts_relational_regression_epoch_incarnation(): string
{
    return str_repeat('c', 32);
}

/** Recover the latest rank statement for structural regression assertions. */
function wp_fts_relational_regression_last_rank_sql(WP_FTS_Relational_Regression_SQLite_WPDB $wpdb): string
{
...
function wp_fts_relational_regression_last_plan_sql(WP_FTS_Relational_Regression_SQLite_WPDB $wpdb): string
{
    for ($index = count($wpdb->queries) - 1; $index >= 0; $index--) {
...
function wp_fts_relational_regression_assert_invalid_cursor(callable $operation, string $message): void
{
    $rejected = false;
    try {
        $operation();
...
/** @return array<string,mixed> */
function wp_fts_relational_regression_work_row(WP_FTS_Relational_Regression_SQLite_WPDB $wpdb, int $postId): array
{
 ...
 *
 * The class name deliberately includes SQLite so relational storage selects
 ...
 */
final class WP_FTS_Relational_Regression_SQLite_WPDB
{
    ...
    /** @var null|callable(string,object[]):array<int,mixed> */
    public mixed $readResultMutator = null;
    ...
    public function get_results(mixed $statement): array
    {
        ...
        try {
            ...
            if (is_callable($this->readResultMutator)) {
                $rows = ($this->readResultMutator)($sql, $rows);
            }
            return $rows;
        } catch (Throwable $error) {
            $this->last_error = $error->getMessage();
    ...
    /** Map the narrow MySQL-dialect forms exercised here onto SQLite syntax. */
