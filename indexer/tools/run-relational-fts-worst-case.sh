...
run_wpcli_php_phase() {
    ...
    local options=()
    while IFS= read -r option; do options+=("${option}"); done < <(env_options "${phase}")
    timed_compose "wpcli-php-${phase}" "$(phase_timeout_seconds "${phase}")" run --rm "${options[@]}" "$@" wpcli --url=http://wordpress --user=admin eval 'ini_set("memory_limit", "128M"); require "/proof/relational-fts-worst-case.php";'
}
