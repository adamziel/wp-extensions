...
/**
 * Four-table relational inverted index for WordPress database adapters.
 *
 ...
 */
final class WP_FTS_Relational_Storage implements WP_FTS_Set_Oriented_Search_Storage
{
    ...
    // Some wpdb drivers expose integral SQL scores as doubles. Keep cursor
    // boundaries inside the largest integer every supported driver can carry
    // exactly, regardless of future ranking-weight changes.
    private const MAX_CURSOR_SCORE = '9007199254740991';
    ...
    public function __construct(object $wpdb, ?string $prefix = null, ?callable $mutationGuard = null)
    {
        ...
        if ($prefix === null) {
            if (!isset($wpdb->prefix) || !is_string($wpdb->prefix)) {
                throw new InvalidArgumentException('Relational storage requires a native WordPress table prefix.');
            }
            $prefix = $wpdb->prefix;
        }
        ...
        if (!$prefixWasExplicit && (!isset($wpdb->posts) || !is_string($wpdb->posts))) {
            throw new InvalidArgumentException('Relational storage requires a native WordPress posts table name.');
        }
        if (!$prefixWasExplicit && (!isset($wpdb->term_relationships) || !is_string($wpdb->term_relationships))) {
            throw new InvalidArgumentException('Relational storage requires a native WordPress term relationships table name.');
        }
        $this->postsTable = $prefixWasExplicit ? $prefix . 'posts' : $wpdb->posts;
        $this->termRelationshipsTable = $prefixWasExplicit ? $prefix . 'term_relationships' : $wpdb->term_relationships;
    ...
    private function verify_schema_from_physical(array $contracts, array $physicalByTable): array
    {
        ...
        foreach ($contracts as $table => $contract) {
            ...
            if ($this->is_sqlite_runtime() && count($physical['indexes']) > count($contract['indexes'])) {
                // SQLite accepts several name variants for one definition.
                // Two valid aliases are still two physical
...
ORDER BY table_name, row_kind, ordinal_position, index_name, index_position"
    ...
    private function normalize_prepared_document(mixed $doc, int|string $offset): array
    {
        ...
        $postId = $doc['doc_id'];
        if (!is_string($doc['primary_lang']) || $doc['primary_lang'] === '' || strlen($doc['primary_lang']) > WP_FTS_Set_Oriented_Search_Storage::MAX_LANGUAGE_BYTES) {
            throw new WP_FTS_Prepared_Document_Rejected(
                $postId,
                'invalid_language',
        ...
        if (
            !is_string($doc['snippet_text'])
            || strlen($doc['snippet_text']) > WP_FTS_Set_Oriented_Search_Storage::MAX_SNIPPET_SOURCE_BYTES
        ) {
            throw new WP_FTS_Prepared_Document_Rejected(
                $postId,
                'invalid_snippet',
    ...
    public function search_page(array $groups, array $options): array
    {
        ...
        $plan = $this->normalize_search_plan($groups);
        if ($plan['groups'] === []) {
        ...
        $rows = $this->normalize_rank_transport_rows(
            $this->get_search_results($statement, 'execute set-oriented FTS page'),
            $includeCanonicalPostRow || $includeMetadata
        );
        ...
        foreach ($visibleRows as $row) {
            ...
            if ($includeMetadata && is_object($detail)) {
                $result += [
                    ...
                    'post_date_gmt' => $detail->post_date_gmt,
                    'title' => $detail->post_title,
                    'excerpt' => $detail->post_excerpt,
                    'primary_lang' => $detail->primary_lang,
                ];
            }
            if ($includeSnippets && is_object($detail)) {
                ...
                $result['primary_lang'] = $detail->primary_lang;
            }
        ...
        if ($includeCanonicalPostRow) {
            // Report the actual private transport map. The earlier SQL length
            // probe excludes PHP field names and exists only to cap hydration.
            foreach ($results as $result) {
                $postRow = $result['_canonical_post_row'];
                $canonicalPageBytes += 8;
                foreach ($postRow as $key => $value) {
                    $canonicalPageBytes += strlen($key);
                    if ($key !== 'ID') {
                        $canonicalPageBytes += strlen($value);
                    }
                }
            }
        }
    ...
    /**
     * Validate the exact row shape returned by the ranking statement.
     *
     * @param object[] $rows
     * @return object[]
     */
    private function normalize_rank_transport_rows(array $rows, bool $withCanonicalBytes): array
    {
        if ($rows === []) {
        ...
        foreach ($rows as $offset => $row) {
            if ($this->search_transport_flag($row, 'snapshot_ready') !== 1) {
                throw new WP_FTS_Search_Unavailable('Full-text search publication changed while ranking the page.');
            }
            $docIdValue = $this->search_transport_property($row, 'doc_id');
            if ($docIdValue === null) {
                if (
                    $offset !== 0
                    || count($rows) !== 1
                    || $this->search_transport_property($row, 'score') !== null
                    || $this->search_transport_property($row, 'post_date_gmt') !== null
                ) {
                    $this->throw_malformed_search_transport('ranking');
                }
                if (
                    $withCanonicalBytes
                    && $this->search_transport_nonnegative_integer($row, 'canonical_post_bytes') !== 0
                ) {
                    $this->throw_malformed_search_transport('ranking');
                }
                continue;
            ...
            $docId = $this->search_transport_positive_integer($row, 'doc_id');
            if (isset($seen[$docId])) {
                $this->throw_malformed_search_transport('ranking');
            }
            $seen[$docId] = true;
            $row->doc_id = $docId;
            $row->score = $this->search_transport_score($row, 'score');
            $row->post_date_gmt = $this->search_transport_date($row, 'post_date_gmt');
            if ($withCanonicalBytes) {
                $row->canonical_post_bytes = $this->search_transport_nonnegative_integer(
                    $row,
    ...
    private function canonical_page_window(array $rows, int $pageSize): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $rowBytes = $row->canonical_post_bytes;
            // Workers reject oversized canonical rows before publishing them.
            // Keep the transport check as defense in depth for a stale index
            // projection or a canonical mutation not yet at its queue hook.
    ...
    {
        $ids = array_map(static fn(object $row): int => $row->doc_id, $rows);
        if ($ids === []) {
            return [];
        }
        ...
        $details = $this->get_search_results($this->wpdb->prepare(
            "/* wp_fts:hydrate */
...
LEFT JOIN ({$detailSql}) hydrated ON snapshot.snapshot_ready = 1",
        ...
        $requiredTextFields = ['primary_lang'];
        if ($includeSnippets) {
            $requiredTextFields[] = 'snippet_text';
        }
        if ($includeMetadata) {
            array_push(
                $requiredTextFields,
        ...
        if ($includeCanonicalPostRow) {
            foreach (WP_FTS_Index_Queue::CANONICAL_POST_COLUMNS as $column) {
                if ($column !== 'ID') {
                    $requiredTextFields[] = 'canonical_post_' . $column;
                }
            }
        }
        ...
        foreach ($details as $detail) {
            if ($this->search_transport_flag($detail, 'snapshot_ready') !== 1) {
                throw new WP_FTS_Search_Unavailable('Full-text search publication changed while hydrating the page.');
            }
            $docIdValue = $this->search_transport_property($detail, 'doc_id');
            if ($docIdValue === null) {
                if (count($details) !== 1) {
                    $this->throw_malformed_search_transport('hydration');
                }
                throw new WP_FTS_Search_Unavailable('Full-text search visibility changed during page hydration.');
            }
            $docId = $this->search_transport_positive_integer($detail, 'doc_id');
            if (!isset($requested[$docId]) || isset($byId[$docId])) {
                $this->throw_malformed_search_transport('hydration');
            }
            $detail->doc_id = $docId;
            foreach ($requiredTextFields as $field) {
                $detail->{$field} = $this->search_transport_text(
                    $detail,
                    $field,
                    str_starts_with($field, 'canonical_post_')
                );
            }
            $byId[$docId] = $detail;
        }
        if (count($byId) !== count($requested)) {
            $this->throw_malformed_search_transport('hydration');
        }
    ...
    private function normalize_search_plan(array $groups): array
    {
        if (!array_is_list($groups)) {
            throw new InvalidArgumentException('FTS logical query groups must be a list.');
        }
        ...
        foreach ($groups as $group) {
            if (!is_array($group) || !array_is_list($group)) {
                throw new InvalidArgumentException('Each FTS logical query group must be a list.');
            }
            $byKey = [];
            $inputAlternatives = 0;
            foreach ($group as $candidate) {
                ...
                if (
                    ...
                    || !array_key_exists('rank', $candidate)
                ) {
                    throw new InvalidArgumentException('Each FTS query alternative must contain exactly key and rank.');
                }
                $key = $candidate['key'];
                if (!is_string($key)) {
                    throw new InvalidArgumentException('Each FTS query alternative key must be a string.');
                }
                if ($key === '' || trim($key) === '') {
                ...
                // One analyzer identity needs one posting range. Preserve its
                // strongest (lowest) rank when repeated occurrences collide.
    ...
    private function assert_search_option_bounds(array $options): void
    {
        ...
        $rawDirection = array_key_exists('direction', $options)
            ? $options['direction']
            : 'after';
        ...
        $hasCursor = array_key_exists('cursor', $options);
        if ($hasCursor) {
            $this->assert_cursor_input_bounds($options['cursor']);
        }
        if (array_key_exists('direction', $options) && !$hasCursor) {
            throw new InvalidArgumentException('FTS cursor direction requires a cursor.');
        }
        foreach (['date_after', 'date_before', 'now_gmt'] as $key) {
            if (!array_key_exists($key, $options)) {
                continue;
            }
            $this->assert_utc_datetime($options[$key], $key);
        }
        foreach (['prefix_matching', 'include_canonical_post_row', 'include_metadata', 'include_snippets', 'explain'] as $key) {
            if (!array_key_exists($key, $options)) {
                continue;
            }
        ...
        if (array_key_exists('prefix_surface', $options)) {
            ...
            foreach (['lang' => WP_FTS_Set_Oriented_Search_Storage::MAX_LANGUAGE_BYTES, 'term' => WP_FTS_Analysis_Limits::MAX_LEXICAL_RUN_BYTES] as $key => $maxBytes) {
    ...
    /** Require the canonical UTC datetime shape supplied by the searcher. */
    private function assert_utc_datetime(mixed $value, string $option): void
    {
        if (
            !is_string($value)
            || $value === ''
            ...
            throw new InvalidArgumentException("FTS {$option} must be a canonical UTC datetime.");
        }
        ...
        if (
            ...
            throw new InvalidArgumentException("FTS {$option} must be a real UTC datetime in Y-m-d H:i:s form.");
        }
    }
    ...
    private function search_prefix_descriptor(array $plan, array $options): ?array
    {
        ...
        $groupId = $options['prefix_group_index'] ?? count($plan['groups']) - 1;
        if (!is_int($groupId) || $groupId < 0 || $groupId >= count($plan['groups'])) {
            throw new InvalidArgumentException('FTS prefix group index must identify an actual logical query group.');
        }
        ...
        foreach ($plan['groups'][$groupId] ?? [] as $candidate) {
            if (hash_equals($identity['lang'], $candidate['lang'])) {
        ...
        $minimum = $options['prefix_min_length'] ?? WP_FTS_Set_Oriented_Search_Storage::DEFAULT_PREFIX_LENGTH;
...
LEFT JOIN {$this->workTable} {$dirty}{$dirtyIndexHint} ON {$dirty}.kind = 'post' AND {$dirty}.post_id = {$postIdExpression}";
        ...
        if (array_key_exists('date_after', $options)) {
            $where[] = "{$post}.post_date_gmt >= %s";
            $args[] = $options['date_after'];
        }
        if (array_key_exists('date_before', $options)) {
            $where[] = "{$post}.post_date_gmt <= %s";
            $args[] = $options['date_before'];
        }
...
WHERE {$range['sql']}",
     ...
     * ordered. All interpolated values have passed the bounded search-option
     * schema; query input never becomes SQL text.
     */
    private function recency_score_expression(array $options, string $postAlias): string
    {
        $strength = $options['recency_boost_strength'] ?? 0.0;
        if ($strength <= 0.0) {
            return 'ranked.score';
        }

        $halfLifeDays = $options['recency_boost_half_life_days'] ?? WP_FTS_Set_Oriented_Search_Storage::DEFAULT_RECENCY_BOOST_HALF_LIFE_DAYS;
        $halfLifeSeconds = (int) round($halfLifeDays * 86400.0);
    ...
    private function normalize_scoring_now(mixed $value): string
    {
        ...
        $this->assert_utc_datetime($value, 'now_gmt');
        return $value;
    }
    ...
    public function commit(): void
    {
        $this->guard_mutation();
        if (!$this->transactionActive) {
            throw new LogicException('No active FTS storage transaction can be committed.');
        }
        try {
    ...
    {
        if (!$this->transactionActive) {
            throw new LogicException('No active FTS storage transaction can be rolled back.');
        }
        try {
    ...
    private function current_search_epoch_for_reset(): int
    {
        ...
        if (
            ...
            $epoch = (int) $value;
            if ($epoch > 0 && (string) $epoch === $value) {
                return $epoch;
            }
        }

        throw new RuntimeException('FTS reset requires one canonical positive search epoch.');
    }
...
CROSS JOIN ({$snapshotSql}) snapshot";
        ...
        [
            ...
            $searchEpochIncarnation,
            $capabilityReady,
            $reconciliationScopeActive,
        ] = $this->normalize_plan_transport_rows($rows, $identities, $prefix !== null);
    ...
    /**
     * Validate every row and cross-row invariant returned by planning.
     *
     * @param object[] $rows
     * @param array<string,array{key:string,lang:string,term:string}> $identities
     * @return array{0:array<string,array<string,mixed>>,1:bool,2:int,3:int,4:string,5:bool,6:bool}
     */
    private function normalize_plan_transport_rows(array $rows, array $identities, bool $expectsSurface): array
    {
        if ($rows === []) {
            $this->throw_malformed_search_transport('planning');
        }

        $first = $rows[0];
        $searchEpoch = $this->search_transport_nonnegative_integer($first, 'search_epoch');
        $searchEpochIncarnation = $this->search_transport_text($first, 'search_epoch_incarnation');
        $capabilityReady = $this->search_transport_flag($first, 'capability_ready') === 1;
        $reconciliationScopeActive = $this->search_transport_flag($first, 'reconciliation_scope_active') === 1;
        $resolved = [];
        $seenExact = [];
        $seenTermIds = [];
        $controlRows = 0;
        ...
        foreach ($rows as $row) {
            if (
                $this->search_transport_nonnegative_integer($row, 'search_epoch') !== $searchEpoch
                || !hash_equals(
                    $searchEpochIncarnation,
                    $this->search_transport_text($row, 'search_epoch_incarnation')
                )
                || ($this->search_transport_flag($row, 'capability_ready') === 1) !== $capabilityReady
                || ($this->search_transport_flag($row, 'reconciliation_scope_active') === 1)
                    !== $reconciliationScopeActive
            ) {
                $this->throw_malformed_search_transport('planning');
            }

            $rowKind = $this->search_transport_text($row, 'row_kind');
            if ($rowKind === 'control') {
                $controlRows++;
                if (
                    $identities !== []
                    || $controlRows !== 1
                    || !$this->search_transport_neutral_plan_row($row)
                ) {
                    $this->throw_malformed_search_transport('planning');
                }
                continue;
            ...
            if ($rowKind === 'surface_available') {
                $surfaceRows++;
                if (
                    !$expectsSurface
                    || $surfaceRows !== 1
                    || !$this->search_transport_neutral_plan_identity($row)
                ) {
                    $this->throw_malformed_search_transport('planning');
                }
                $surfaceFlag = $this->search_transport_flag($row, 'surface_available');
                $surfaceFrequency = $this->search_transport_nonnegative_integer($row, 'surface_doc_freq');
                if ($surfaceFlag === 0 && $surfaceFrequency !== 0) {
                    $this->throw_malformed_search_transport('planning');
                }
                $surfaceAvailable = $surfaceFlag === 1;
                $surfaceDocFreq = $surfaceFrequency;
                continue;
            }

            if ($rowKind !== 'exact' || $identities === []) {
                $this->throw_malformed_search_transport('planning');
            }
            if (
                $this->search_transport_nonnegative_integer($row, 'kind') !== self::LEXICAL_KIND
                || $this->search_transport_flag($row, 'surface_available') !== 0
                || $this->search_transport_nonnegative_integer($row, 'surface_doc_freq') !== 0
            ) {
                $this->throw_malformed_search_transport('planning');
            }
            $lang = $this->search_transport_text($row, 'lang');
            $term = $this->search_transport_text($row, 'term');
            try {
            ...
            if (
                $expected === null
                || isset($seenExact[$key])
                || !hash_equals($expected['lang'], $lang)
                || !hash_equals($expected['term'], $term)
            ) {
                $this->throw_malformed_search_transport('planning');
            }
            $seenExact[$key] = true;

            $termIdValue = $this->search_transport_property($row, 'term_id');
            $docFreqValue = $this->search_transport_property($row, 'doc_freq');
            if ($termIdValue === null) {
                if ($docFreqValue !== null) {
                    $this->throw_malformed_search_transport('planning');
                }
                continue;
            }
            $termId = $this->search_transport_positive_integer($row, 'term_id');
            if (isset($seenTermIds[$termId])) {
                $this->throw_malformed_search_transport('planning');
            }
            $seenTermIds[$termId] = true;
            $resolved[$key] = [
                'term_id' => $termId,
                'doc_freq' => $this->search_transport_nonnegative_integer($row, 'doc_freq'),
                'lang' => $expected['lang'],
                'term' => $expected['term'],
            ];
        }
        ...
        if (
            count($seenExact) !== count($identities)
            || ($identities === [] ? $controlRows !== 1 : $controlRows !== 0)
            || ($expectsSurface ? $surfaceRows !== 1 : $surfaceRows !== 0)
        ) {
            $this->throw_malformed_search_transport('planning');
        }
    ...
    /** The control row contains no dictionary or surface result. */
    private function search_transport_neutral_plan_row(object $row): bool
    {
        return $this->search_transport_neutral_plan_identity($row)
            && $this->search_transport_flag($row, 'surface_available') === 0
            && $this->search_transport_nonnegative_integer($row, 'surface_doc_freq') === 0;
    }

    /** Surface and control rows use the exact neutral dictionary identity. */
    private function search_transport_neutral_plan_identity(object $row): bool
    {
        return $this->search_transport_nonnegative_integer($row, 'term_id') === 0
            && $this->search_transport_text($row, 'lang') === ''
            && $this->search_transport_nonnegative_integer($row, 'kind') === 0
            && $this->search_transport_text($row, 'term') === ''
            && $this->search_transport_nonnegative_integer($row, 'doc_freq') === 0;
    }

    /** Execute a search read without silently dropping malformed driver rows. */
    private function get_search_results(mixed $statement, string $context): array
    {
        $rows = $this->wpdb->get_results($statement);
        $this->assert_no_database_error($context);
        if (!is_array($rows) || !array_is_list($rows)) {
            $this->throw_malformed_search_transport($context);
        }
        foreach ($rows as $row) {
            if (!is_object($row)) {
                $this->throw_malformed_search_transport($context);
    ...
    /** Read one required result alias without treating absence as NULL. */
    private function search_transport_property(object $row, string $property): mixed
    {
        if (!property_exists($row, $property)) {
            $this->throw_malformed_search_transport('database row');
        }

        return $row->{$property};
    }

    /** Read a canonical nonnegative database integer. */
    private function search_transport_nonnegative_integer(object $row, string $property): int
    {
        return $this->search_transport_integer_value(
            $this->search_transport_property($row, $property),
            false
        );
    }

    /** Read a canonical positive database integer. */
    private function search_transport_positive_integer(object $row, string $property): int
    {
        return $this->search_transport_integer_value(
            $this->search_transport_property($row, $property),
            true
    ...
    /** Accept only native integers or their canonical unsigned decimal form. */
    private function search_transport_integer_value(mixed $value, bool $positive): int
    {
        if (is_int($value)) {
            $integer = $value;
        } elseif (
            ...
            $this->throw_malformed_search_transport('database integer');
        }
        if ($integer < ($positive ? 1 : 0)) {
            $this->throw_malformed_search_transport('database integer');
        }
    ...
    /** Read an exact SQL boolean represented as zero or one. */
    private function search_transport_flag(object $row, string $property): int
    {
        $flag = $this->search_transport_nonnegative_integer($row, $property);
        if ($flag > 1) {
            $this->throw_malformed_search_transport('database flag');
        }

        return $flag;
    }

    /** Preserve one integral ranking score as canonical decimal text. */
    private function search_transport_score(object $row, string $property): string
    {
        try {
            return $this->normalize_cursor_score($this->search_transport_property($row, $property));
        } catch (InvalidArgumentException) {
            $this->throw_malformed_search_transport('ranking score');
        }
    }

    /** Read one exact UTF-8 database string, with optional native integer transport. */
    private function search_transport_text(object $row, string $property, bool $allowInteger = false): string
    {
        $value = $this->search_transport_property($row, $property);
        if ($allowInteger && is_int($value)) {
            $value = (string) $value;
        }
        if (!is_string($value) || preg_match('//u', $value) !== 1) {
            $this->throw_malformed_search_transport('database text');
        }

        return $value;
    }

    /** Read one exact WordPress GMT datetime or its all-zero sentinel. */
    private function search_transport_date(object $row, string $property): string
    {
        $date = $this->search_transport_text($row, $property);
        if ($date === '0000-00-00 00:00:00') {
            return $date;
        }
        if (preg_match('/^([0-9]{4})-([0-9]{2})-([0-9]{2}) ([0-9]{2}):([0-9]{2}):([0-9]{2})$/D', $date, $parts) !== 1) {
            $this->throw_malformed_search_transport('ranking date');
        }
        if (
    ...
    /** Search result transport corruption cannot be treated as an empty match. */
    private function throw_malformed_search_transport(string $context): never
    {
        throw new WP_FTS_Search_Unavailable("Full-text search received malformed {$context} transport.");
    }
...
{$gateJoin} ({$range['sql']}) surface_identity{$gatePredicate}";
    ...
    private function insert_term_identities(array $identities, array $documentCounts = []): void
    {
        ...
        // SQLite distinguishes TEXT and BLOB comparison operands. The
...
ON DUPLICATE KEY UPDATE primary_lang=VALUES(primary_lang),content_hash=VALUES(content_hash),snippet_text=VALUES(snippet_text),indexed_at=VALUES(indexed_at)",
    ...
    private function normalize_filter_values(mixed $values): array
    {
        if (!is_array($values) || !array_is_list($values)) {
            throw new InvalidArgumentException('FTS filters must be lists of strings.');
        }
        if (count($values) > WP_FTS_Set_Oriented_Search_Storage::MAX_FILTER_VALUES) {
            throw new InvalidArgumentException('FTS filters accept at most 32 values.');
        }
        $normalized = [];
        foreach ($values as $value) {
            if (!is_string($value) || $value === '' || trim($value) !== $value) {
                throw new InvalidArgumentException('Each FTS filter value must be a nonempty unpadded string.');
            }
            ...
            if (isset($normalized[$value])) {
                throw new InvalidArgumentException('FTS filter values must not repeat.');
            }
        ...
        if (!empty($options['explain'])) {
            ...
            $page['explain'] = [
                ...
                'recency_boost' => [
                    'enabled' => $recencyStrength > 0.0,
                    'strength' => $recencyStrength,
                    'half_life_days' => $options['recency_boost_half_life_days'] ?? WP_FTS_Set_Oriented_Search_Storage::DEFAULT_RECENCY_BOOST_HALF_LIFE_DAYS,
                    'scoring_now_gmt' => '',
                ],
